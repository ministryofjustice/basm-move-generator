...
const person = ({ firstName, lastName, dob, gender, ethnicity, prison_number, police_national_computer }) => ({
  data: {
    type: 'people',
    attributes: {
...
module.exports = {
  ...
  staticPerson: ({ prison_number, police_national_computer } = {}) => ({
    firstName: 'Cookie-Alias',
    lastName: 'Cutter',
    prison_number,
    police_national_computer,
    dob: format(new Date(1980, 0, 1), 'yyyy-MM-dd'),
    gender: { male: 'ffac6763-26d6-4425-8005-6e5d052aed88' },
    ethnicity: { asian_british_indian: '9894def5-2997-4c46-affa-e868412e2d0e' },
  }),

  randomPerson: ({ firstName, lastName, prison_number, police_national_computer, dob, gender } = {}) =>
    person({
      firstName: firstName || faker.person.firstName(),
      lastName: lastName || faker.person.lastName(),
      prison_number,
      police_national_computer,
      dob: dob || format(faker.date.between({ from: '1980-01-01T01:01:01.000Z', to: '2004-01-01T01:01:01.000Z' }), 'yyyy-MM-dd'),
      gender: gender || random(data.gender),
      ethnicity: random(data.ethnicity),
    }),

  moves: {
    remand: ({ profile, date, fromLocation, toLocation }) => ({
      data: {
        type: 'moves',
        attributes: {
    ...
    recall: ({ profile, date, fromLocation, toLocation }) => ({
      data: {
        type: 'moves',
        attributes: {
          date: format(date, 'yyyy-MM-dd'),
    ...
    transfer: ({ profile, date, fromLocation, toLocation }) => ({
      data: {
        type: 'moves',
        attributes: {
  ...
  journeys: {
    proposed: ({ fromLocation, toLocation }) => ({
      data: {
        type: 'journeys',
        attributes: {
          state: 'proposed',
    ...
    inProgress: ({ occurredAt, journeyId }) => ({
      data: {
        type: 'events',
        attributes: {
