class ReferenceDataLookup {
  ...
  async findPeople() {
    ...
    console.log(
      people.filter(p => Boolean(p.prisonNumber)).filter(p => p.prisonNumber.match(/[A-Z][0-9]{4}[A-Z]{1,2}/g)),
    )
  }

  async findPrisons() {
    const prisons = await this.api.getRaw('/api/reference/locations?per_page=100&filter[location_type]=prison')

    console.log(
      prisons
        .filter(p => !p.disabledAt)
        .map(({ id, title }) => ({
          [title.toLowerCase().replace(/\s+/g, '_').replace(/[()]/g, '').replace(/[&]/g, 'and').replace(/\//g, '_')]:
            id,
        })),
    )
  }
  ...
  async findCourts() {
    const courts = await this.api.getRaw('/api/reference/locations?per_page=1000&filter[location_type]=court')
    ...
    const sortedSlice = shuffled.slice(0, 50).sort((a, b) => Object.keys(a)[0].localeCompare(Object.keys(b)[0]))
    console.log(sortedSlice)
  }

  async findPolice() {
    const police = await this.api.getRaw('/api/reference/locations?per_page=1000&filter[location_type]=police')
  ...
  async findEthnicities() {
    const ethnicities = await this.api.getRaw('/api/reference/ethnicities?per_page=100')
