const { moves, journeys, random, data, profile, syncProfile, randomPerson } = require('../data/models')
...
class BasmService {
  ...
  async findPersonByPnc(pnc) {
    ...
    const { id, firstNames, lastName, dateOfBirth, gender } = person
  ...
  async createPerson(args = {}) {
    ...
    const { id, firstNames, lastName, dateOfBirth } = person
  ...
  async getPersonId(prisonNumber) {
    ...
    const { id, firstNames, lastName, dateOfBirth, gender } = result
    this.logger.info(`Found person: ${firstNames} ${lastName} (${dateOfBirth}, ${gender})`)
    return id
  }
}
