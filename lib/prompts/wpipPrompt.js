...
module.exports = async (generator, basmApi) => {
  ...
  if (mode === ModeEnum.GENERATE) {
    ...
    if (scenarios.includes(ScenariosEnum.MANUAL_MULTIPLE_MATCH)) await generator.manualMultipleMatch();
    if (scenarios.includes(ScenariosEnum.AUTOMATIC_MULTIPLE_MATCH)) await generator.automaticMultipleMatch();
